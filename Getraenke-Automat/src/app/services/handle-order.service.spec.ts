...
describe('HandleOrderService', () => {
  ...
  it('should check if order is valid', () => {
    expect(service.handleOrder(5, '5')).toBeTrue();
    expect(service.handleOrder(3, '2')).toBeTrue();
    expect(service.handleOrder(6, '1')).toBeTrue();
    expect(service.handleOrder(3, '5')).toBeTrue();
  });
